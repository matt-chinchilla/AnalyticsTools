Sub CompleteQBMacro()
    ...
    ' Check if the headers are already in the first row
    Dim areHeadersInFirstRow As Boolean
    areHeadersInFirstRow = True
    For Each header In Array("Debit", "Credit", "Amount")
        If ws.Cells(1, 1).End(xlToRight).Find(What:=header, LookIn:=xlValues, LookAt:=xlWhole) Is Nothing Then
            areHeadersInFirstRow = False
            Exit For
        End If
    Next header

    ' Delete rows above the first occurrence of the headers only if they're not in the first row
    If Not areHeadersInFirstRow Then
        ...
        For Each header In Array("Debit", "Credit", "Amount")
            ...
            End If
        Next header
        If firstRow > 1 Then ws.Rows("1:" & firstRow - 1).Delete
    End If
    ...
    If debitsCol > 0 And creditsCol > 0 Then
        ...
        Set targetWorkbook = Workbooks.Open("C:\Users\*YourFilePath*")
        On Error Resume Next
